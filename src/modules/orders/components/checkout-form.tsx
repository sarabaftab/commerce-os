...
export function CheckoutForm({ tenantSlug, preview }: CheckoutFormProps) {
  ...
  const errorMessage = state.error
    ? /required|invalid|fill/i.test(state.error)
  ...
  return (
    <form action={formAction} className="space-y-4 pb-28">
      ...
      <div className="fixed inset-x-0 bottom-0 z-20 border-t border-[color:var(--shop-line)] bg-[color:var(--shop-bg)]/95 backdrop-blur">
        <div
          className="mx-auto w-full max-w-lg px-4 py-3 sm:max-w-2xl md:max-w-3xl lg:max-w-5xl"
          style={{
            paddingBottom:
