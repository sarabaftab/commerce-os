 ...
 * Never returns blank solely because a translation is missing.
 */
export function localizedValue(input: {
  ...
  if (input.locale === "zh" && !isBlank(input.zh)) {
...
export function localizedContent(
  locale: Locale,
  fields: { en: string; km?: string | null; zh?: string | null },
): string {
  return localizedValue({ locale, ...fields });
}

export function assertNeverBlankLocalized(
  value: string,
  fallbackEn: string,
): string {
